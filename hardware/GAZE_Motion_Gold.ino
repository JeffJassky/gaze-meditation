...
float var_noiseGate = 0.04;		  // N: Raw G-force threshold (0.1G * 10 = 1.0 magnified)
...
void processCommand(String cmd)
{
	...
	else if (key == 'I')
	{
		...
		if (var_reportInterval < 1)
			var_reportInterval = 1;
	...
	else if (key == 'Z')
	{
		...
		if (var_windowSize > WINDOW_SIZE_MAX)
			var_windowSize = WINDOW_SIZE_MAX;
		if (var_windowSize < 2)
			var_windowSize = 2;
...
void analyzeAndSend()
{
	...
	{
		float *activeBuffer = (rangeX >= rangeY && rangeX >= rangeZ) ? histX : (rangeY >= rangeX) ? histY
																								  : histZ;
		float sum = 0;
		for (int i = 0; i < var_windowSize; i++)
			sum += activeBuffer[i];
		...
		{
			if (isAbove && activeBuffer[i] < (mean - var_freqHyst))
			{
				isAbove = false;
				crossings++;
			}
			else if (!isAbove && activeBuffer[i] > (mean + var_freqHyst))
			{
				isAbove = true;
				crossings++;
			}
