 ...
 */
import type { SceneConfig, SceneBehaviorConfig, BehaviorSuggestion, BehaviorSuggestionRaw } from '@shared/types'
...
// The editor manipulates suggestions generically (dynamic type changes,
// catalog-driven defaults), so we work with BehaviorSuggestionRaw internally
// and let the discriminated union enforce correctness at consumption sites.
const suggestions = computed<BehaviorSuggestionRaw[]>(
	() => (behavior.value.suggestions ?? []) as BehaviorSuggestionRaw[],
)
...
function removeBehaviorSuggestion(index: number) {
	...
	next.splice(index, 1)
	write('suggestions', next as BehaviorSuggestion[])
}

function updateBehaviorSuggestion(index: number, patch: Partial<BehaviorSuggestionRaw>) {
	const next = suggestions.value.slice()
