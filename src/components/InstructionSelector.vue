    ...
    >
      ...
      <div class="overflow-y-auto flex-1">
        <div 
          ...
          :key="item.instr.id"
          @click="emit('select', item.originalIndex); isExpanded = false"
          class="cursor-pointer p-2 hover:bg-zinc-700 rounded mb-1 transition-colors text-left"
          :class="{ 'bg-cyan-900/50 border border-cyan-700': item.originalIndex === currentIndex }"
        >
          <div class="font-bold flex justify-between items-center mb-1">
            <span class="text-cyan-500 text-[10px] font-mono uppercase tracking-wider" :title="item.instr.constructor.name">
          ...
          <div 
            ...
            :title="Array.isArray(item.instr.options.text) ? item.instr.options.text.join(' ') : (item.instr.options.text || item.instr.options.prompt)"
          >
            {{ Array.isArray(item.instr.options.text) ? item.instr.options.text.join(' ') : (item.instr.options.text || item.instr.options.prompt) }}
          </div>
          <div v-else class="text-zinc-500 italic text-left">
