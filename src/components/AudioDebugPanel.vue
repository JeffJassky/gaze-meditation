...
const handleClickOutside = (e: MouseEvent) => {
	if (isMixerOpen.value && audioDebugRoot.value && !audioDebugRoot.value.contains(e.target as Node)) {
...
<template>
	...
	>
		...
		>
			<svg v-if="masterVolume === 0" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>
			<svg v-else-if="masterVolume < 0.5" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>
			<svg v-else xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>
		</button>
		...
		<Transition name="fade-slide">
			...
			>
				...
				<div class="bg-zinc-900/90 border border-zinc-700 p-2 rounded-full shadow-xl backdrop-blur-sm flex flex-col items-center gap-2">
				...
				<!-- Full Mixer Panel -->
				<div v-if="isMixerOpen" class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-zinc-900/95 border border-zinc-700 p-4 rounded-xl shadow-2xl w-64 backdrop-blur-md text-xs font-mono">
					<!-- Buses -->
					<div class="space-y-3 mb-4">
						<div v-for="(vol, bus) in busVolumes" :key="bus">
							<div class="flex justify-between mb-1 text-zinc-400 text-[10px] uppercase tracking-wider">
								<span>{{ bus }}</span>
					...
					<div class="border-t border-zinc-700 pt-3 mt-2">
						<div class="flex justify-between items-center mb-2">
							<span class="font-bold text-white text-[10px] tracking-wider">BINAURAL</span>
							<span class="text-[10px]" :class="binauralState.active ? 'text-green-400' : 'text-zinc-600'">
								{{ binauralState.active ? 'ACTIVE' : 'OFF' }}
							</span>
						</div>
						<div v-if="binauralState.active" class="space-y-1 text-zinc-400 text-[10px]">
