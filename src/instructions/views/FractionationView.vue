<template>
	...
	>
		<div class="content">
			...
			<h1
				class="fade-in"
				v-else-if="instruction.status.value === 'WAITING_FOR_OPEN'"
			>
				OPEN EYES
			</h1>
			<h1
				class="fade-in"
				v-else-if="instruction.status.value === 'OPEN'"
			>
				OPEN
			</h1>
			<h1
				class="fade-in"
				v-else-if="instruction.status.value === 'WAITING_FOR_CLOSED'"
			>
				CLOSE EYES
			</h1>
			<h1
				class="fade-in"
				v-else-if="instruction.status.value === 'CLOSED'"
			>
				CLOSE
			</h1>
			<h1
				class="fade-in"
				v-else-if="instruction.status.value === 'FINISHED'"
			>
				COMPLETE
			</h1>
			...
			<p
				class="cycle-info"
				v-if="
					instruction.status.value !== 'READY' && instruction.status.value !== 'FINISHED'
				"
